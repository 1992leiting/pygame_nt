...
def create_account(account_name, passwd):
    ...
    if os.path.exists(account_path):
        send(server.tmp_client_socket[account_name], S_系统提示, dict(内容='账号已存在,无法继续创建'))
    ...
    send(server.tmp_client_socket[account_name], S_系统提示, dict(内容='账号创建成功'))
    sprint('账号已创建:{}'.format(account_name))


def account_login(account, passwd) -> bool:
    """
    账号登陆, 返回是否成功
    :param account:
    :param passwd:
...
def send_hero_data_by_account(account):
    """
    登陆时, 发送这个账号所有的人物数据
    :param account:
    :return:
    ...
    send(server.tmp_client_socket[account], S_账号所有人物, dict(内容=heroes, 账号=account))
